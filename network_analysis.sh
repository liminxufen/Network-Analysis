...
function check_package_manager() {
    ...
    if cat /etc/issue | grep -q -E -i "ubuntu|debian"; then
        systemPackage='apt'
    elif cat /etc/issue | grep -q -E -i "cnetos|red hat|redhat|tlinux"; then
        systemPackage='yum'
    elif cat /proc/version | grep -q -E -i "ubuntu|debian"; then
        systemPackage='apt'
    elif cat /proc/version | grep -q -E -i "centos|red hat|redhat|tlinux"; then
        systemPackage='yum'
    else
        echo "unkown"
    fi
...
function realTimeTraffic() {
    ...
    while true; do
        ...
        #清除当前行
        printf "\033[K"
        printf "%-20s %-20s\n" "Receive: $(bit_to_readable $eth_in)" "$(bit_to_readable $eth_in_peak)"
        #清除当前行
        printf "\033[K"
        printf "%-20s %-20s %-20s\n" "Transmit: $(bit_to_readable $eth_out)" "$(bit_to_readable $eth_out_peak)"
        [[ $clear == true ]] && clear=false
    done    
}

#流量和连接概览
function FlowAndConnectionProfile() {
    if ! which tcpdump > /dev/null; then
        echo "tcpdump not found, going to install it."
        if check_package_manager apt; then #确定包管理工具
    ...
    #tcpdump监听网络
    tcpdump -v -i $eth -tnn > ./tcpdump_temp 2>&1 &
    sleep 10
    clear
    ...
    echo "$eth Receive: $(bit_to_readable $eth_in)/s"
    echo "$eth Transmit: $(bit_to_readable $eth_out)/s"
    echo

    local regTcpdump=$(ifconfig | grep -A 1 $eth | awk -F'[: ]+' '$0~/inet addr:/{printf $4"|"}' | sed -e 's/|$//' -e 's/^/(/' -e 's/$/)\\\\\.[0-9]+:/')
    #新旧版本tcpdump输出格式不一样,分别处理
    if awk '/^IP/{print;exit}' ./tcpdump_temp | grep -q ")$"; then
        #处理tcpdump文件
        awk '/^IP/{print;getline;print}' ./tcpdump_temp > ./tcpdump_temp2
    else
        #处理tcpdump文件
        awk '/^IP/{print}' ./tcpdump_temp > ./tcpdump_temp2
        sed -i -r 's#(.*: [0-9]+\))(.*)#\1\n  \2#' ./tcpdump_temp2
    fi    

    awk '{len=$NF;sub(/\)/,"",len);getline;print $0,len}' ./tcpdump_temp2 > ./tcpdump

    #统计每个端口在10s内的平均流量
    echo -e "\033[32maverage traffic in 10s base on server port: \033[0m"
    awk -F'[ .:]+' -v regTcpdump=$regTcpdump '{if ($0 ~ regTcpdump){line="clients > "$8"."$9"."$10"."$11":"$12}else{line=$2"."$3"."$4"."$5":"$6" > clients"};sum[line]+=$NF*8/10}END{for (line in sum){printf "%s %d\n", line,sum[line]}}' ./tcpdump | \
    sort -k 4 -nr | head -n 10 | while read a b c d; do
        echo "$a $b $c $(bit_to_readable $d)/s"
    done
    echo -ne "\033[11A"
    echo -ne "\033[50C"
    echo -e "\033[32maverage traffic in 10s base on client port: \033[0m"
    awk -F '[ .:]+' -v regTcpdump=$regTcpdump '{if ($0 ~ regTcpdump){line=$2"."$3"."$4"."$5":"$6" > server"}else{line="sever > "$8"."$9"."$10"."$11":"$12};sum[line]+=$NF*8/10}END{for (line in sum){printf "%s %d\n", line,sum[line]}}' ./tcpdump | \
    sort -k 4 -nr | head -n 10 | while read a b c d; do
        echo -ne "\033[50C"
        echo "$a $b $c $(bit_to_readable $d)/s"
    done    
    ...
    echo -ne "\033[11A"
    echo -ne "\033[50C"
    echo -e "\033[32mtop 10 ip average traffic in 10s base one client: \033[0m"
    ...
    #统计连接状态
    local regSS=$(ifconfig | grep -A 1 $eth | awk -F'[: ]+' '$0~/inet addr:/{print $4"|"}' | sed -e 's/|$//') 
    ss -an | grep -v -E "LISTEN|UNCONN" | grep -E "$regSS" > ./ss
    echo -e "\033[32mconnecttion state count: \033[0m"
    awk 'NR>1{sum[$(NF-4)]+=1}END{for (state in sum){print state,sum[state]}}' ./ss | sort -k 2 -nr 
    echo

    #统计各端口连接状态
    echo -e "\033[32mconnection state count by port base on server: \033[0m"
    awk 'NR>1{sum[$(NF-4),$(NF-1)]+=1}END{for (key in sum){split(key,subkey,SUBSEP);print subkey[1],subkey[2],sum[subkey[1],subkey[2]]}}' ./ss | sort -k 3 -nr | head -n 10
    echo -ne "\033[11A"
    echo -ne "\033[50C"
    echo -e "\033[32mconnection state count by port base on client: \033[0m"
    awk 'NR>1{sum[$(NF-4),$(NF)]+=1}END{for (key in sum){split(key,subkey,SUBSEP);print subkey[1],subkey[2],sum[subkey[1],subkey[2]]}}' ./ss | sort -k 3 -nr | head -n 10 | awk '{print "\033[50C"$0}'
    echo

    #统计端口为80且状态为ESTAB连接数最多的前10个IP
    echo -e "\033[32mtop 10 ip ESTAB state count at port 80: \033[0m"
    cat ./ss | grep ESTAB | awk -F'[: ]+' '{sum[$(NF-2)]+=1}END{for (ip in sum){print ip,sum[ip]}}' | sort -k 2 -nr | head -n 10
    echo

    #统计端口为80且状态为SYN-RECV连接数最多的前10个IP
    echo -e "\033[32mtop 10 ip SYN-RECV state count at port 80: \033[0m"
    cat ./ss | grep -E "$regSS" | grep SYN-RECV | awk -F'[: ]+' '{sum[$(NF-2)]+=1}END{for (ip in sum){print ip,sum[ip]}}' | sort -k 2 -nr | head -n 10
}
    ...
    while true; do
        ...
        case $select in
            1) realTimeTraffic;break
               ;;
            2) FlowAndConnectionProfile;break 
               ;;
